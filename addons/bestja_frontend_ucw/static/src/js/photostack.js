...
// BES-158 add links to images
		this.figures = [].slice.call( document.getElementById( 'photostack-1' ).getElementsByTagName( 'figure' ) );
		this.figures.forEach( function( figure, idx ) {
		...
		// rotate the photo when clicking on the back side or background
		// but only on small screens
		if (window.innerWidth < 767 ){
		document.getElementById( 'photostack-1' ).addEventListener( 'click', function() {
			if(self.flipped)
				self._rotateItem();
		} );
		}
		...
		this.nav.style.marginLeft = ((0 - navWidth) / 2) + "px";
// end add links to images BES-158
